...
export default function DashboardFileExplorer({ scope = 'workspace' }: { scope?: 'workspace' | 'ready' }) {
  ...
  function showContextMenu(event: React.MouseEvent, item: ExplorerItem) {
    ...
    setContextItem(item);
    setMenuPosition({ x: Math.max(8, Math.min(event.clientX, window.innerWidth - 260)), y: Math.max(8, Math.min(event.clientY, window.innerHeight - 430)) });
  }
  ...
  return (
    <section className="relative rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
      ...
      <div className="flex h-[calc(100vh-11rem)] min-h-[580px] overflow-hidden">
        <aside className="flex w-72 shrink-0 flex-col border-r dark:border-gray-700">
          <div className="min-h-0 flex-1 overflow-y-auto p-3">
            <div className="mb-2 flex items-center justify-between gap-2"><h3 className="text-sm font-bold">Categories</h3><div className="flex items-center gap-1"><button type="button" disabled={selectedCategoryIds.size === 0} onClick={() => setSelectedCategoryIds(new Set())} className="rounded px-2 py-1 text-xs font-bold text-gray-600 hover:bg-gray-100 disabled:opacity-40">Clear</button><button type="button" onClick={() => createCategory(null)} className="rounded px-2 py-1 text-xs font-bold text-blue-600 hover:bg-blue-50">+ New</button></div></div>
      ...
      {contextItem && (
        <div className="fixed z-50 max-h-[calc(100vh-16px)] min-w-52 max-w-[calc(100vw-16px)] overflow-y-auto rounded-lg border bg-white py-1 shadow-xl dark:bg-gray-800" style={{ left: menuPosition.x, top: menuPosition.y }} onMouseLeave={() => setContextItem(null)}>

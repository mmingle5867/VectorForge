...
export default function ArtworkPreviewTunePage() {
  ...
  const [layoutDpi, setLayoutDpi] = useState(300);
  ...
  const setImageDimension = (axis: 'width' | 'height', value: number) => { if (!Number.isFinite(value) || value <= 0) return; const ratio = width && height ? width / height : imageWidth / imageHeight; const rounded = Math.max(1, Math.round(value)); if (axis === 'width') { setImageWidth(rounded); if (width) setResizePercent(Number((rounded / width * 100).toFixed(2))); if (maintainAspect && ratio) setImageHeight(Math.max(1, Math.round(rounded / ratio))); } else { setImageHeight(rounded); if (height) setResizePercent(Number((rounded / height * 100).toFixed(2))); if (maintainAspect && ratio) setImageWidth(Math.max(1, Math.round(rounded * ratio))); } };
  ...
  useEffect(() => { if (width && height) { setImageWidth(width); setImageHeight(height); setCanvasWidth(width); setCanvasHeight(height); } }, [width, height]);
    ...
    try {
      const response = await fetch(`/api/artwork/${artworkId}/output`, {
        ...
        body: JSON.stringify({
          ...
          specification: {
            constrainBy: 'WIDTH', value: canvasWidth / layoutDpi, unit: 'IN',
            dpi: layoutDpi, formats: outputFormats,
          },
        }),
      });
      const data = await response.json();
      if (!response.ok || !data.success) throw new Error(data.error || 'Unable to create output files'); await refresh(false); setOutputsCreated(true);
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <main className="grid gap-6 xl:grid-cols-[1fr_380px]"><div className="space-y-6">
      ...
      {jpegReady && <section className="border-t pt-4">
        ...
        <p className="mt-2 text-xs text-gray-600">Outputs use the prepared canvas: {canvasWidth.toLocaleString()} × {canvasHeight.toLocaleString()} px · {(canvasWidth / layoutDpi).toFixed(2)} × {(canvasHeight / layoutDpi).toFixed(2)} in at {layoutDpi} DPI.</p>

...
export async function POST(
  ...
  try {
    ...
    if (!item.uploadPath) {
      return NextResponse.json(
        { success: false, error: 'Item no longer has an upload file path' },
        { status: 400 }
      );
    }
    ...
    await prisma.$transaction([
      ...
      prisma.batch.update({
        ...
        data: {
          status: 'PROCESSING',
          completedItems,
          failedItems: Math.max(0, item.batch.failedItems - 1),
          startedAt: new Date(),
          completedAt: null,
        },
      }),
    ...
    logger.info(`Retry started for failed item ${itemId}`, {
      batchId,
      itemId,
      userId: user.id,
    });
    ...
    return NextResponse.json({
      success: true,
      message: `Retry started for ${item.baseName}`,
      resetItems: 1,

...
export default function ArtworkPreviewTunePage() {
  ...
  const canCreateSelectedImages = hasSelectedImageFormats && jpegReady && (recreateSvg || rasterOutputDimensionsValid);
  ...
  async function exportRasterOutputs() {
    setActionError(null);
    if (!hasSelectedImageFormats) {
      setActionError('Check at least one image format first. A checked box means create or recreate that format.');
      return;
    }
    if (!jpegReady) {
      setActionError('Click Ready to Continue after preparing the working JPG, then create the checked image formats.');
      return;
    }
    if (outputFormats.length > 0 && !rasterOutputDimensionsValid) {
      setActionError('Enter valid canvas width, height, and resolution before creating JPG, PNG, PNG Mask, or PDF output.');
      return;
    }
    setNotice('Creating the checked image formats…');
    setBusy(true);
    try {

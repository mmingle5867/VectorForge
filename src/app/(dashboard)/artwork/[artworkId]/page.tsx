...
type PreviewKind = 'svg' | 'png' | 'pngMask' | 'jpg';
...
export default function ArtworkPreviewTunePage() {
  ...
  const [outputsCreated, setOutputsCreated] = useState(false);
  ...
  async function loadVersions(preserve = false) {
    ...
    setJpegReady(data.jpegReady === true); setHasWorkingPng(Boolean(data.workingPng?.filePath)); setAvailableOutputs(data.rasterOutputs || []); setOutputsCreated((data.rasterOutputs || []).length > 0);
  ...
  async function prepareLayoutAndOpen() {
    ...
    try {
      ...
      const data = await response.json(); if (!response.ok || !data.success) throw new Error(data.error || 'Unable to prepare image');
      await fetch('/api/local-editor/open', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'original-raster', artworkId, prepare: false }) });
      setJpegReady(false); setOutputsCreated(false); await refresh(false); setNotice('Created a new prepared working JPG and opened it in the raster editor.');
    } catch (error) { setActionError(error instanceof Error ? error.message : 'Unable to prepare image'); } finally { setBusy(false); }
  }
  ...
  async function markJpegReady() { setActionError(null); setBusy(true); try { const response = await fetch(`/api/artwork/${artworkId}/working-jpg/ready`, { method: 'POST' }); const data = await response.json(); if (!response.ok || !data.success) throw new Error(data.error || 'Unable to mark JPG ready'); setJpegReady(true); setHasWorkingPng(true); setOutputsCreated(false); setExpandedStage('outputs'); setRevision(Date.now()); setNotice(data.pngCreated ? 'Working JPG is ready. A working PNG mask was created; now select output formats.' : 'Working JPG is ready. Now select output formats.'); } catch (error) { setActionError(error instanceof Error ? error.message : 'Unable to mark JPG ready'); } finally { setBusy(false); } }
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <main className="grid gap-6 xl:grid-cols-[1fr_380px]"><div className="space-y-6">
      ...
      {vectorSvgUrl && <section className="rounded-xl border border-emerald-300 bg-white p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h2 className="font-bold">Vector review — {vectorView === 'pngMask' ? 'PNG Mask' : vectorView.toUpperCase()}</h2><div className="flex flex-wrap gap-2"><button onClick={() => setVectorView('svg')} className={`rounded border px-3 py-1 text-sm font-semibold ${vectorView === 'svg' ? 'bg-gray-200' : ''}`}>View SVG</button><button onClick={() => setVectorView('png')} className={`rounded border px-3 py-1 text-sm font-semibold ${vectorView === 'png' ? 'bg-gray-200' : ''}`}>View PNG</button><button onClick={() => setVectorView('pngMask')} className={`rounded border px-3 py-1 text-sm font-semibold ${vectorView === 'pngMask' ? 'bg-gray-200' : ''}`}>View PNG Mask</button><button onClick={() => setVectorView('jpg')} className={`rounded border px-3 py-1 text-sm font-semibold ${vectorView === 'jpg' ? 'bg-gray-200' : ''}`}>View JPG</button></div></div>{(vectorView === 'png' || vectorView === 'pngMask') && <label className="mb-3 flex items-center gap-2 text-sm font-semibold">Review background <input type="color" value={pngBackground} onChange={(event) => setPngBackground(event.target.value)} className="h-8 w-12 rounded border p-1" /></label>}{vectorView === 'svg' ? <PreviewSurface key={`${vectorCandidateId}-svg`} src={vectorSvgUrl} alt="Provisional SVG trace" checkerboard /> : vectorView === 'png' && availableOutputs.some((output) => output.format === 'PNG') ? <PreviewSurface key={`${revision}-output-png`} src={outputPngUrl} alt="Generated PNG" background={pngBackground} /> : vectorView === 'pngMask' && availableOutputs.some((output) => output.format === 'PNG_MASK') ? <PreviewSurface key={`${revision}-output-png-mask`} src={outputPngMaskUrl} alt="Generated PNG mask" background={pngBackground} /> : vectorView === 'jpg' ? <PreviewSurface key={`${revision}-jpg`} src={workingJpegUrl} alt="Working JPG" /> : <p className="rounded border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">Create the matching selected output file to review it here.</p>}<p className="mt-2 text-xs text-gray-500">The background color is review-only; it is never written into either PNG file.</p></section>}

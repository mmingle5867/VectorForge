...
export async function exportRasterOutputs(input: { sourcePath: string; outputDirectory: string; baseName: string; spec: RasterOutputSpecification }) {
  ...
  if (input.spec.formats.includes('PNG')) { const target = path.join(input.outputDirectory, `${input.baseName}.png`); await image.clone().png().toFile(target); outputs.push({ format: 'PNG', path: target, mimeType: 'image/png' }); }

  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <main className="grid gap-6 xl:grid-cols-[1fr_380px]"><div className="space-y-6">
      ...
      {jpegReady && <section className="border-t pt-4"><StageHeader title="3. Select additional output formats" expanded={expandedStage === 'outputs'} onToggle={() => setExpandedStage((current) => current === 'outputs' ? null : 'outputs')} />{expandedStage === 'outputs' && <>
        ...
        <button disabled={busy || outputFormats.length === 0 || canvasWidth < 1 || canvasHeight < 1 || canvasDpi < 1} onClick={() => void exportRasterOutputs()} className="mt-3 w-full rounded bg-sky-700 px-3 py-2 font-semibold text-white disabled:opacity-50">Create selected output files</button>{availableOutputs.length > 0 && <div className="mt-3 flex flex-wrap gap-2"><span className="w-full text-xs font-semibold">Open / inspect outputs</span><button type="button" disabled={busy} onClick={() => void openOutputFolder()} className="rounded border px-3 py-1 text-sm font-semibold">Open Output Folder</button>{availableOutputs.filter((output) => output.format === 'JPG' || output.format === 'PNG' || output.format === 'PNG_MASK').map((output) => <button key={output.format} type="button" disabled={busy} onClick={() => void openGeneratedOutput(output.format as 'JPG' | 'PNG' | 'PNG_MASK')} className="rounded border px-3 py-1 text-sm font-semibold">{output.format === 'PNG_MASK' ? 'PNG Mask' : output.format}</button>)}</div>}
      </>}</section>}
